...
const ReportCard: React.FC<ReportCardProps> = ({ student, stats, settings, onSettingChange, onStudentUpdate, classAverageAggregate, totalEnrolled, isFacilitator }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    // Configure to fit strictly on one sheet
    const opt = { 
      margin: 0, 
  ...
  return (
    <div id={`report-${student.id}`} className="bg-white p-6 max-w-[210mm] mx-auto h-[297mm] border border-gray-100 shadow-2xl print:shadow-none print:border-none page-break relative flex flex-col box-border font-sans mb-10 overflow-hidden">
       ...
       <div data-html2canvas-ignore="true" className="absolute top-4 right-4 no-print flex gap-2 z-[60]">
          <button onClick={handleShareWhatsApp} className="w-10 h-10 bg-green-500 text-white rounded-xl shadow-lg flex items-center justify-center hover:scale-110 transition-all"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 1 1-7.6-7.6 8.38 8.38 0 0 1 3.8.9L21 3.5Z"/></svg></button>
          <button onClick={handleDownloadPDF} disabled={isGenerating} className={`w-10 h-10 rounded-xl shadow-lg flex items-center justify-center transition-all ${isGenerating ? 'bg-gray-400' : 'bg-red-600 text-white hover:scale-110'}`}>
       ...
       {/* Optimized Header (Reduced Vertical Space) */}
       <div className="mb-2">
       ...
       <div className="grid grid-cols-2 gap-4 mb-3 border-2 border-blue-900 p-3 rounded-2xl bg-blue-50/5 text-[11px] font-bold">
       ...
       <div className="mt-auto pt-2 border-t border-gray-100">
          <div className="bg-slate-900 text-white p-3 rounded-xl flex justify-between items-center gap-4">
             <div className="flex-1">
                ...
                <p className="text-[7px] leading-tight opacity-60">Grades derived relative to cohort mean (μ). BSA calculated from 4 Core + 2 best Electives. Lower aggregate signifies exponential proficiency.</p>
             </div>
             <div className="text-[7px] font-mono opacity-30 text-right uppercase">Network Verified Shard<br/>{student.id.toString(16).toUpperCase()}</div>
          </div>
       </div>

       {/* Signatures (Reduced padding) */}
       <div className="flex justify-between items-end mt-4 pb-2 border-t border-gray-100 pt-3">
