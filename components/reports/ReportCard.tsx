    ...
    <div id={`report-${student.id}`} className="bg-white p-6 w-[210mm] mx-auto h-[297mm] border border-gray-100 shadow-2xl print:shadow-none print:border-none page-break relative flex flex-col box-border font-sans mb-10 overflow-hidden shrink-0">
       ...
       <div className="grid grid-cols-2 gap-4 mb-4 border-2 border-blue-900 p-4 rounded-2xl bg-blue-50/5 text-[12px] font-bold shrink-0">
          ...
          <div className="space-y-1.5 pl-4">
            ...
            <div className="flex items-center justify-between"><span className="text-gray-400 uppercase text-[10px]">Academic Level:</span><span className={`px-3 py-0.5 rounded text-white text-[9px] font-black uppercase ${student.category === 'Distinction' ? 'bg-green-600' : 'bg-blue-600'}`}>{student.category}</span></div>
          </div>
       </div>
       ...
       <div className="mb-4 flex-1 overflow-hidden">
         <table className="w-full text-[11px] border-collapse border-2 border-blue-900">
            <thead className="bg-blue-950 text-white uppercase text-[9px] tracking-widest">
              <tr>
                <th className="py-2.5 px-4 text-left">Academic Discipline</th>
                <th className="py-2.5 px-1 text-center">Obj</th>
                <th className="py-2.5 px-1 text-center">Thy</th>
                <th className="py-2.5 px-1 text-center">SBA</th>
                <th className="py-2.5 px-1 text-center bg-blue-900">Total</th>
                <th className="py-2.5 px-1 text-center">Grd</th>
                <th className="py-2.5 px-4 text-left">Facilitator</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
               {student.subjects.map(sub => (
                 <tr key={sub.subject} className="even:bg-gray-50/30 font-bold h-8">
                   <td className="px-4 py-1 text-blue-950 uppercase truncate max-w-[150px]">{sub.subject}</td>
                   ...
                   <td className="px-4 py-1 text-[9px] font-black text-blue-800 uppercase truncate opacity-60 italic">{sub.facilitator}</td>
                 </tr>
               ))}
            </tbody>
         </table>
       ...
       {/* REMARKS SECTOR */}
       <div className="grid grid-cols-1 gap-3 mb-4 text-[11px] shrink-0">
          <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 shadow-sm">
             <span className="font-black text-blue-900 uppercase block text-[9px] mb-1 tracking-widest">Performance Shard Analysis:</span>
             <p className="italic text-gray-700 leading-relaxed font-bold">"{dynamicAnalysis.performance}"</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-blue-50/30 p-4 rounded-xl border border-blue-100"><span className="font-black text-blue-900 uppercase block text-[9px] mb-1 tracking-widest">Conduct & Character:</span><p className="font-black text-blue-800 uppercase italic leading-tight">"{student.conductRemark || 'EXEMPLARY CHARACTER OBSERVED.'}"</p></div>
            <div className="bg-indigo-50/30 p-4 rounded-xl border border-indigo-100"><span className="font-black text-indigo-900 uppercase block text-[9px] mb-1 tracking-widest">Mastery Guidance:</span><div className="text-indigo-950 font-black italic leading-tight"><EditableField value={student.overallRemark} onChange={(v) => onStudentUpdate?.(student.id, v)} multiline={true} placeholder={dynamicAnalysis.recommendation} className="w-full border-none text-[11px]" /></div></div>
          </div>
       </div>

       {/* LOGIC APPENDIX */}
       <div className="mt-auto pt-3 border-t border-gray-100 shrink-0">
          <div className="bg-slate-900 text-white p-4 rounded-2xl flex justify-between items-center gap-6">
             <div className="flex-1">
                <h5 className="text-[8px] font-black uppercase tracking-[0.2em] mb-1">NRT Grading Logic (σ-Spread: {settings.useTDistribution ? 'T-Dist' : 'Normal Z'})</h5>
                <p className="text-[8px] leading-tight opacity-70">Grades derived relative to cohort mean (μ). BSA calculated from 4 Core + 2 best Electives. Lower aggregate signifies proficiency.</p>
             </div>
             <div className="text-[9px] font-mono opacity-30 text-right uppercase">Network Shard<br/>{student.id.toString(16).toUpperCase().padStart(2, '0')}</div>
          </div>
       </div>

       {/* VALIDATION SIGNATURES */}
       <div className="flex justify-between items-end mt-6 pb-2 border-t border-gray-100 pt-4 shrink-0">
         <div className="w-[30%] text-center border-t-2 border-gray-900 pt-2">
            ...
            <div className="font-black text-blue-900 text-[10px] uppercase truncate">
               <EditableField value={settings.headTeacherName} onChange={(v) => onSettingChange('headTeacherName', v)} className="text-center" />
            </div>
         </div>
         <div className="w-[30%] text-center border-t-2 border-gray-900 pt-2">
            <p className="text-[9px] font-black uppercase text-gray-400 tracking-widest mb-1">Resumption Protocol</p>
