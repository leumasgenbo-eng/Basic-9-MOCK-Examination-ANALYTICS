...
const LoginPortal: React.FC<LoginPortalProps> = ({ onLoginSuccess, onSuperAdminLogin, onFacilitatorLogin, onPupilLogin, onSwitchToRegister }) => {
  ...
  const handleVerifyPin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (email.toLowerCase() === 'leumasgenbo4@gmail.com') {
        ...
        onFacilitatorLogin(metadata.name, metadata.subject, hubId);
      } else if (role === 'pupil') {
  ...
  return (
    <div className="w-full max-w-lg p-4 animate-in fade-in zoom-in-95 duration-500">
      <div className="bg-slate-950 p-10 md:p-14 rounded-[3.5rem] shadow-2xl border border-white/10 relative overflow-hidden">
        ...
        <div className="text-center relative mb-12">
          ...
          <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em] mt-3">United Baylor Academy Network</p>
        </div>
        ...
        {step === 'EMAIL' ? (
          <form onSubmit={handleRequestPin} className="space-y-6">
            ...
            {error && <div className="bg-red-500/10 text-red-500 p-4 rounded-2xl text-[10px] font-black uppercase text-center border border-red-500/20">{error}</div>}
            ...
            >
              {isLoading ? "REQUESTING PIN..." : "GET ACCESS PIN"}
            </button>
            
            <div className="pt-6 text-center border-t border-white/5">
              <button type="button" onClick={onSwitchToRegister} className="text-[9px] font-black text-blue-400 uppercase tracking-widest hover:text-white">Register New Institution</button>
            </div>
          </form>
        ) : (
          <form onSubmit={handleVerifyPin} className="space-y-8 animate-in slide-in-from-right-4 duration-500">
            <div className="text-center space-y-2">
              <h3 className="text-xl font-black text-white uppercase tracking-tight">Enter Secure PIN</h3>
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Sent to {email}</p>
            </div>
            ...
            <input 
              ...
              className="w-full bg-slate-900 border border-white/5 rounded-2xl px-6 py-6 text-3xl font-black text-white text-center tracking-[0.5em] outline-none focus:ring-4 focus:ring-blue-500/10" 
              placeholder="000000" 
              maxLength={6}
              required 
              autoFocus
